@9.8a_ChecktheRecordFiltersinClientServiceRequestsTab
Feature: Check with the Record filters available under Client Service Requests Records Tab
  ...
  Scenario: Navigate to Client Service Requests Records Tab and check with all filters available on dashbaord
    ...
    Given I populate picker field "Requestor" with partially matching suggestions for
      | user1 |
    Given I wait for "8" seconds
    Given I populate field "Account Manager" with "$$$AccountManager$$$"
    Given I wait for "8" seconds
    ...
    Given I wait for "5" seconds
    Given I populate field "To[1]" with "$$$date$$$"
    Given I wait for "5" seconds
    ...
    Given I wait for "5" seconds
    Given I populate field "To" with "$$$date$$$"
    Given I wait for "5" seconds
    #Clear the filters
    ...
    Given I wait for "5" seconds
    Given I verify grid "[1]" column "[13]" row "[1]" contains "$$$AccountManager$$$"
    Given I wait for "5" seconds
    Given I verify grid "[1]" column "[12]" row "[1]" contains "$$$CSAUSER1$$$"
    Given I wait for "5" seconds
